...
interface PricelistAuditLogProps {
  pricelistId: number;
  show: boolean;
  onHide: () => void;
}

export default function PricelistAuditLog({
  pricelistId,
  show,
  onHide,
}: PricelistAuditLogProps) {
  const [auditLogs, setAuditLogs] = useState<{
    pricelistItemAudits: AuditLogEntry[];
    itemAudits: AuditLogEntry[];
  }>({ pricelistItemAudits: [], itemAudits: [] });
  ...
  const fetchAuditLogs = async () => {
    ...
    try {
      const result = await apiCall(`/api/menu/pricelists/${pricelistId}/audit`);
      if (result.status === 200) {
        setAuditLogs(result.data || { pricelistItemAudits: [], itemAudits: [] });
      } else {
  ...
  const allLogs = [
    ...auditLogs.pricelistItemAudits.map((log) => ({ ...log, type: "Pricelist Item" })),
    ...auditLogs.itemAudits.map((log) => ({ ...log, type: "Item" })),
  ].sort((a, b) => new Date(b.changed_at).getTime() - new Date(a.changed_at).getTime());

  return (
    <Modal show={show} onHide={onHide} size="xl">
      <Modal.Header closeButton>
        <Modal.Title>Audit Log - Pricelist #{pricelistId}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <ErrorDisplay
          error={error}
