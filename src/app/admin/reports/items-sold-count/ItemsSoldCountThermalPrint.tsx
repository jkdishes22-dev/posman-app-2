...
// Column widths: 22 + 2sep + 3 + 2sep + 11 = 40 chars total
const COL_NAME = 22;
const COL_QTY = 3;
const COL_AMT = 11;
const SEP = "  ";
...
function itemRow(name: string, qty: number, amount: number): string {
  return padRight(truncate(name, COL_NAME), COL_NAME) + SEP + padLeft(String(qty), COL_QTY) + SEP + padLeft(fmtAmount(amount), COL_AMT);
}
...
export default function ItemsSoldCountThermalPrint({ bill }: { bill?: PrintData }) {
  ...
  lines.push(centerTextLine("Items Sold Count Report", W));
  lines.push(centerTextLine("(Cumulative)", W));
  lines.push("");
  ...
  lines.push(rulerLine("-", W));
  lines.push(padRight("TOTAL SALES", W - COL_AMT) + padLeft(fmtAmount(totalAmt), COL_AMT));
  lines.push(rulerLine("=", W));
