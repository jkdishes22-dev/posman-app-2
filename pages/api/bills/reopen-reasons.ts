...
const getReopenReasons = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
...
const createReopenReason = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
...
const updateReopenReason = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        const { id } = req.query;
...
const deleteReopenReason = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        const { id } = req.query;
        if (!id || isNaN(Number(id))) {
            return res.status(400).json({
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    if (req.method === "GET") {
        return getReopenReasons(req, res);
    } else if (req.method === "POST") {
        return authorize([permissions.CAN_EDIT_BILL])(createReopenReason)(req, res);
    } else if (req.method === "PATCH") {
        return authorize([permissions.CAN_EDIT_BILL])(updateReopenReason)(req, res);
    } else if (req.method === "DELETE") {
        return authorize([permissions.CAN_EDIT_BILL])(deleteReopenReason)(req, res);
    } else {
