...
const getProductionStockRevenue = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const report = await reportService.getProductionStockRevenueReport(filters);
    res.status(200).json({ reports: report });
  } catch (error: any) {
    console.error("Production Stock Revenue Report API error:", error);
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  await authorize([permissions.CAN_VIEW_PRODUCTION_STOCK_REVENUE_REPORT])(getProductionStockRevenue)(req, res);
};
