...
const getProductionSalesReconciliation = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const reportService = new ReportService(req.db);
    const { startDate, endDate, itemId, period } = req.query;
    ...
    const report = await reportService.getProductionSalesReconciliationReport(filters);
    res.status(200).json({ reports: report });
  } catch (error: any) {
    console.error("Production Sales Reconciliation Report API error:", error);
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  await authorize([permissions.CAN_VIEW_PRODUCTION_SALES_RECONCILIATION_REPORT])(getProductionSalesReconciliation)(req, res);
};
