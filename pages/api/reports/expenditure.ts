...
const getExpenditure = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const reportService = new ReportService(req.db);
    const { startDate, endDate, itemId, supplierId, period } = req.query;
    ...
    const report = await reportService.getExpenditureReport(filters);
    res.status(200).json({ reports: report });
  } catch (error: any) {
    console.error("Expenditure Report API error:", error);
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  await authorize([permissions.CAN_VIEW_EXPENDITURE_REPORT])(getExpenditure)(req, res);
};
