...
async function replaceWindowsSqliteBinary(context, destNodeModules, projectRoot, log, err) {
    ...
    log(`\n   🔄 Ensuring Windows better_sqlite3.node (${arch}, electron ABI ${abi})...`);
    log(`   📥 URL: ${tarUrl}`);

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bsq3-win-"));
    const tarPath = path.join(tmpDir, "archive.tar.gz");
    const cacheDir = path.join(projectRoot, ".cache", "better-sqlite3");
    const cachedTarPath = path.join(cacheDir, tarFileName);

    try {
        if (fs.existsSync(cachedTarPath)) {
            log(`   📦 Using cached prebuilt: ${cachedTarPath}`);
            fs.copyFileSync(cachedTarPath, tarPath);
        } else {
            execSync(`curl -L --fail -o "${tarPath}" "${tarUrl}"`, { stdio: "pipe" });
            fs.mkdirSync(cacheDir, { recursive: true });
            fs.copyFileSync(tarPath, cachedTarPath);
            log(`   💾 Cached prebuilt for offline reuse: ${cachedTarPath}`);
        }
        ...
        err(`   ❌ Could not install better_sqlite3.node for Electron ABI ${abi}: ${e.message}`);
        err(`   💡 Offline build tip: pre-seed the exact prebuilt tarball at: ${cachedTarPath}`);
        err(`   💡 Expected tarball name: ${tarFileName}`);
